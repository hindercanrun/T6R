...
LUI.createMenu.DPadArea = function ( f1_arg0 )
	...
	self:setLeftRight( false, true, -CoD.DPadArea.CircleSize, 0 )
	self:setTopBottom( false, true, -CoD.DPadArea.CircleSize, 0 )
	self.id = "DpadCircle"
	local f1_local2 = 10
	...
	self.circleBackground = LUI.UIImage.new()
	self.circleBackground:setLeftRight( true, true, 0, 0 )
	self.circleBackground:setTopBottom( true, true, 0, 0 )
	self.circleBackground:setImage( CoD.DPadArea.CircleBackgroundMaterial )
	self:addElement( self.circleBackground )
	CoD.OffhandIcons.Width = CoD.OffhandIcons.Width_Zombie
	self:addElement( CoD.AmmoCounter.new( {
		left = 0,
		top = 0,
		right = 0,
		bottom = 0,
	...
	self:addElement( CoD.OtherAmmoCounters.new( {
		left = 0,
		top = 0,
		right = 0,
		bottom = 0,
	...
	local f1_local3 = -88
	local f1_local4 = -131
	self:addElement( CoD.WeaponLabel.new( {
		left = f1_local4 - 100,
	...
	local f1_local5 = -1
	local f1_local6 = -1
	self:addElement( CoD.OffhandIcons.new( "lethal", {
	...
	self:addElement( CoD.OffhandIcons.new( "tactical", {
	...
	self.carouselArrows = LUI.UIElement.new( {
		left = 0,
		top = 0,
		right = 0,
		bottom = 0,
	...
	self:addElement( self.carouselArrows )
	local f1_local7 = 8
	local f1_local8 = 8
	local f1_local9 = 4
	self.carouselArrows:addElement( LUI.UIImage.new( {
		left = -f1_local7 / 2,
	...
	self:addElement( inventoryWeapon )
	self.inventoryWeapon = inventoryWeapon
	...
	self.inventoryWeaponIcon = inventoryWeaponIcon
	...
	self:registerEventHandler( "hud_update_refresh", CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_HUD_VISIBLE, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_EMP_ACTIVE, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_DEMO_CAMERA_MODE_MOVIECAM, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_DEMO_ALL_GAME_HUD_HIDDEN, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_IN_VEHICLE, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_IN_GUIDED_MISSILE, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_IN_REMOTE_KILLSTREAK_STATIC, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_AMMO_COUNTER_HIDE, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_IS_FLASH_BANGED, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_UI_ACTIVE, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_SPECTATING_CLIENT, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_SCOREBOARD_OPEN, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_PLAYER_DEAD, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_IS_SCOPED, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_bit_" .. CoD.BIT_IS_PLAYER_ZOMBIE, CoD.DPadArea.UpdateVisibility )
	self:registerEventHandler( "hud_update_actionslots", CoD.DPadArea.UpdateActionSlots )
	self:registerEventHandler( "hud_update_inventory_weapon", CoD.DPadArea.UpdateInventoryWeapon )
	self:registerEventHandler( "hud_fade_dpad", CoD.DPadArea.UpdateFading )
	self:registerEventHandler( "hud_update_team_change", CoD.DPadArea.UpdateTeamChange )
	if CoD.isPC then
		self:registerEventHandler( "input_source_changed", CoD.DPadArea.InputSourceChanged )
		if CoD.useController and Engine.LastInput_Gamepad() then
			CoD.DPadArea.InputSourceChanged( self, {
				source = 0
			} )
		else
			CoD.DPadArea.InputSourceChanged( self, {
...
CoD.DPadArea.UpdateActionSlots = function ( f2_arg0, f2_arg1 )
	...
	for f2_local4, f2_local9 in pairs( f2_arg1.actionSlotData ) do
		...
		if f2_local7 ~= nil then
			self = LUI.UIElement.new( f2_local7 )
			if self ~= nil then
				f2_arg0:addElement( self )
				f2_arg0.actionSlots[f2_local4] = self
				self:addElement( LUI.UIImage.new( {
					left = 0,
					top = 0,
					right = 0,
				...
				if CoD.isPC then
					...
					if f2_local8 ~= nil then
						self.slotIndex = f2_local4
						self.keyPrompt = LUI.UIText.new( f2_local8 )
						self.keyPrompt:setRGB( CoD.yellowGlow.r, CoD.yellowGlow.g, CoD.yellowGlow.b )
						self.keyPrompt:setFont( CoD.fonts.Condensed )
						self.keyPrompt:setTopBottom( false, false, -CoD.textSize.Default / 2, CoD.textSize.Default / 2 )
						self.keyPrompt:setLeftRight( false, true, -f2_local11 - 120, -f2_local11 - 10 )
						self.keyPrompt:setAlpha( 0.5 )
						self.keyPrompt:setAlignment( LUI.Alignment.Right )
						self:registerAnimationState( "KeyPrompt", f2_local8 )
						self:addElement( self.keyPrompt )
						if CoD.useController and Engine.LastInput_Gamepad() then
							CoD.DPadArea.ActionSlotInputSourceChanged( self, {
								source = 0
							} )
						else
							CoD.DPadArea.ActionSlotInputSourceChanged( self, {
