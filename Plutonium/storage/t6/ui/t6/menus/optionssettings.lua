...
CoD.OptionsSettings.CreateGraphicsTab = function (GraphicsTab, LocalClientIndex)
	...
	local DisplayResolutionChoices = GraphicsTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("VIDEO MODE"), "r_mode")
	CoD.OptionsSettings.Button_AddChoices_Resolution(DisplayResolutionChoices)
	
	local DisplayModeChoices = GraphicsTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("DISPLAY MODE"), "r_fullscreen")
	CoD.OptionsSettings.Button_AddChoices_DisplayMode(DisplayModeChoices)
	if DisplayModeChoices:getCurrentValue() == "2" then
		DisplayResolutionChoices:disableSelector()
	end
	
	local MonitorUsedChoices = GraphicsTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("PLATFORM_MONITOR_CAPS"), "r_monitor")
	...
	local MaxFpsChoices = GraphicsTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("PLATFORM_MAX_FPS_CAPS"), "com_maxfps")
	...
	CoD.Options.Button_AddChoices_YesOrNo(GraphicsTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("MENU_SYNC_EVERY_FRAME_CAPS"), "r_vsync"))

	GraphicsTabButtonList:addSpacer(CoD.CoD9Button.Height / 2)

	CoD.OptionsSettings.Button_AddChoices_TextureFiltering(GraphicsTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("MENU_TEXTURE_MIPMAPS_CAPS"), "r_texFilterQuality"))

	local AntiAliasingChoices = GraphicsTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("MENU_ANTIALIASING_CAPS"), "r_aaSamples")
	...
	local TextureQualityChoices = GraphicsTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("MENU_TEXTURE_QUALITY_CAPS"), "r_picmip")
	...
	local ShadowChoices = GraphicsTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("MENU_SHADOWS_CAPS"), "sm_spotQuality")
	...
	CoD.Options.Button_AddChoices_OnOrOff(GraphicsTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("PLATFORM_AMBIENT_OCCLUSION_CAPS"), "r_ssao"))
	CoD.OptionsSettings.Button_AddChoices_DepthOfField(GraphicsTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("PLATFORM_DEPTH_OF_FIELD_CAPS"), "r_dofHDR"))

	local BrightnessChoices = GraphicsTabButtonList:addButton(Engine.Localize("MENU_BRIGHTNESS_CAPS"))
...
CoD.OptionsSettings.CreateSoundTab = function (SoundTab, LocalClientIndex)
	...
	CoD.Options.Button_AddChoices_OnOrOff(SoundTabButtonList:addProfileLeftRightSelector(LocalClientIndex, Engine.Localize("LISTENING LEVEL"), "snd_menu_hearing_impaired"))
	if UIExpression.DvarBool(nil, "sd_can_switch_device") == 0 then
	else
		CoD.AudioSettings.Button_AudioPresets_AddChoices(SoundTabButtonList:addProfileLeftRightSelector(LocalClientIndex, Engine.Localize("SPEAKER SETUP"), "snd_menu_presets", "", nil, nil, CoD.AudioSettings.CycleSFX))
...
CoD.OptionsSettings.CreateVoiceChatTab = function (f44_arg0, f44_arg1)
	...
	CoD.OptionsSettings.Button_AddChoices_VoiceChat(f44_local1:addHardwareProfileLeftRightSelector(Engine.Localize("VOICE CHAT"), "cl_voice"))
	local f44_local4 = f44_local1:addVoiceMeter(Engine.Localize("LEVEL INDICATOR"))
...
CoD.OptionsSettings.CreateGameTab = function (GameTab, LocalClientIndex)
	...
	local FOVSlider = CoD.OptionsSettings.AddDvarLeftRightSlider(GameTabButtonList, LocalClientIndex, Engine.Localize("PLATFORM_FIELD_OF_VIEW_CAPS"), "cg_fov_default", 65, 120)
	FOVSlider:setNumericDisplayFormatString("%d")
	
	local FOVScaleSlider = GameTabButtonList:addDvarLeftRightSlider(LocalClientIndex, Engine.Localize("FOV SCALE"), "cg_fovscale", 0.5, 2)
	FOVScaleSlider:setNumericDisplayFormatString("%.2f")
	FOVScaleSlider:setRoundToFraction(0.05)
	FOVScaleSlider:setBarSpeed(0.01)
	
	local FOVSensitivity = GameTabButtonList:addDvarLeftRightSelector(LocalClientIndex, Engine.Localize("FOV SENSITIVITY"), "cg_usefovsensitivity")
	...
	CoD.OptionsSettings.Button_AddChoices_StreamerMode(GameTabButtonList:addHardwareProfileLeftRightSelector("STREAMER MODE", "cl_enableStreamerMode"))
	
	local SafeAreaButton = GameTabButtonList:addButton(Engine.Localize("MENU_SAFE_AREA_ADJUSTMENT_CAPS"))
	SafeAreaButton:setActionEventName("open_safe_area")

	GameTabButtonList:addSpacer(CoD.CoD9Button.Height / 2)
	
	local UnlockAllButton = GameTabButtonList:addButton(Engine.Localize("UNLOCK ALL"))
	UnlockAllButton:setActionEventName("open_unlockall")

	local ResetStatsButton = GameTabButtonList:addButton(Engine.Localize("RESET STATS"))
	ResetStatsButton:setActionEventName("open_resetstats")

	return GameTabContainer
end

CoD.OptionsSettings.CreateZombiesTab = function (ZombiesTab, LocalClientIndex)
	local ZombiesTabContainer = LUI.UIContainer.new()
	local InGame = UIExpression.IsInGame() == 1
	local ZombiesTabButtonList = CoD.Options.CreateButtonList()
	ZombiesTab.buttonList = ZombiesTabButtonList
	ZombiesTabContainer.buttonList = ZombiesTabButtonList
	ZombiesTabContainer:addElement(ZombiesTabButtonList)
	local TextureQualityChoices = ZombiesTabButtonList:addHardwareProfileLeftRightSelector(Engine.Localize("MENU_TEXTURE_QUALITY_CAPS"), "r_picmip")
	CoD.OptionsSettings.Button_AddChoices_TextureQuality(TextureQualityChoices)
	if Engine.GetHardwareProfileValueAsString("r_picmip_manual") == "0" then
		TextureQualityChoices:setChoice(-1)
	end
...
LUI.createMenu.OptionsSettingsMenu = function (LocalClientIndex)
	...
	OptionsSettingsWidget:registerEventHandler("open_unlockall", CoD.PartyLobby.OpenConfirmUnlockAll)
	OptionsSettingsWidget:registerEventHandler("open_resetstats", CoD.PartyLobby.OpenConfirmResetStats)
	...
	SettingsTabs:addTab(LocalClientIndex, "MENU_ZOMBIES_CAPS", CoD.OptionsSettings.CreateZombiesTab)
...
CoD.OptionsSettings.OpenConfirmUnlockAll = function (OptionsSettingsWidget, ClientInstance)
	OptionsSettingsWidget:saveState()
	OptionsSettingsWidget:openMenu("unlockall", ClientInstance.controller)
	OptionsSettingsWidget:close()
end

CoD.OptionsSettings.OpenConfirmResetStats = function (OptionsSettingsWidget, ClientInstance)
	OptionsSettingsWidget:saveState()
	OptionsSettingsWidget:openMenu("resetstats", ClientInstance.controller)
	OptionsSettingsWidget:close()
end